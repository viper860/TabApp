...
var SassBuilder = {
    ...
    onBuildSassFile: function(update){
        ...
        builder.build(update, function(css, err){
    ...
    onInvalidateSassFile: function(update){
        ...
        if(sassFile) {
            console.log("invalidating sass file " + path);
            sassFile.invalidate();
        } else {
            console.log("SassFile not found for path " + path);
        }
    },
...
(function(){
    ...
    Fashion.apply(Fashion, {
        ...
        getSavedVariables: function() {
            var setters = SassBuilder && SassBuilder.getSavedVariables && SassBuilder.getSavedVariables(),
                varMap = Fashion.lastBuilder.getContext().dynamicsMap,
                sources = {};
            if(setters) {
                setters = JSON.parse(setters);
                for (var name in setters) {
                    var jsName = Fashion.getJsName(name);
                    if (jsName in varMap) {
                        var text = this.getNodeValue(varMap[jsName].node);
                        if (text) {
                            sources[name] = text;
                        }
                    }
        ...
        reExecute: function (vars, callback, skipReload) {
            ...
            css.getText(function(css) {
                if (!skipReload) {
                    builder.injectCss(builder.lastCssPath, css);
                }
                callback && callback();
            }, true);
        },

        setVariables: function (vars, callback, skipReload) {
            vars = Fashion.createVariables(vars);
